...
class FlowTests {
    ...
    private fun preAuthorizationTest(): UniqueIdentifier {
        ...
        networkOperatorNode.startFlow(AssignRoleFlow(networkId, membershipIds[insurer]!!, CustomRole.INSURER_CLAIMS_OFFICER.name))
        networkOperatorNode.startFlow(AssignRoleFlow(networkId, membershipIds[hospital]!!, CustomRole.HOSPITAL_REGISTRAR.name))
        networkOperatorNode.startFlow(AssignRoleFlow(networkId, membershipIds[patient]!!, CustomRole.PATIENT.name))
        network.runNetwork()
    ...
    private fun paymentReceiptTest(preAuthorizationLinearId: UniqueIdentifier) {
        ...
        // Assign roles to different parties.
        val membershipIds = membershipStateAndRef.associateBy({ it.state.data.identity.cordaIdentity }, { it.state.data.linearId })
        networkOperatorNode.startFlow(AssignRoleFlow(networkId, membershipIds[insurer]!!, CustomRole.INSURER_FINANCE_CLERK.name))
        networkOperatorNode.startFlow(AssignRoleFlow(networkId, membershipIds[hospital]!!, CustomRole.HOSPITAL_FINANCE_CLERK.name))
        network.runNetwork()
