    ...
    override fun verify(tx: LedgerTransaction) {
        ...
        tx.commands.forEach {
            ...
            when (it.value) {
                is Commands.Create -> {
                    requireThat {
                        "Payer in outputState must be the same as payer in preAuthorizationState" using
                                (outputState!!.payer == preAuthorizationState!!.policyIssuer)
                        "Payee in outputState must be the same as payee in preAuthorizationState" using
